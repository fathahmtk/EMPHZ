...
const IndustryCard: React.FC<{ industry: typeof INDUSTRIES[0] }> = ({ industry }) => {
    ...
    return (
        <Link
            ...
            className={`block relative overflow-hidden rounded-lg shadow-md group transform transition-transform duration-300 hover:-translate-y-1 hover:shadow-xl opacity-0 ${isVisible ? 'animate-fadeInScaleUp' : ''}`}
        >
            <img
                src={industry.image}
                alt={industry.name}
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent group-hover:from-black/90 transition-colors duration-300 flex items-end p-6">
