...
const MobileCollapsible: React.FC<{title: string; children: React.ReactNode}> = ({title, children}) => {
    ...
    return (
        <div>
            <button onClick={() => setIsSubOpen(!isSubOpen)} className="w-full flex justify-between items-center text-[var(--color-primary)] hover:bg-gray-100 rounded-md font-medium py-2 px-3 text-left">
                <span>{title}</span>
