      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {BRAND_PILLARS.map((pillar, index) => (
            <div key={index} className="bg-[var(--color-surface)] p-8 rounded-lg shadow-[var(--shadow-md)] flex flex-col items-center text-center transition-all duration-300 transform hover:-translate-y-2 border border-[var(--color-border)] hover:shadow-[var(--shadow-xl)]">
              ...
              <p className="text-[var(--color-secondary)]">{pillar.description}</p>
            </div>
          ))}
        </div>
      </div>
