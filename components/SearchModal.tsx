          ...
          {isIndexLoading ? (
            ...
            <>
              <div className="p-2 bg-[var(--color-surface)] border-b border-[var(--color-border)] flex items-center justify-between text-sm sticky top-0 z-10">
                ...
                <select value={sortOrder} onChange={e => setSortOrder(e.target.value as any)} className="text-sm rounded border-[var(--color-border)] focus:ring-2 focus:ring-[var(--color-brand)]/50 focus:border-[var(--color-brand)] p-1 bg-[var(--color-background)] text-[var(--color-text-primary)]">
              ...
              <ul>
                {sortedResults.map((result, index) => (
                  <li key={`${result.type}-${result.code}`}>
                    <Link to={result.link} onClick={onClose} className={`flex items-start gap-4 p-4 border-b border-gray-100 transition-colors duration-150 ${activeIndex === index ? 'bg-[var(--color-brand)]/10' : 'hover:bg-gray-50'}`}>
                      <div className={`mt-1 flex-shrink-0 rounded-md flex items-center justify-center h-8 w-8 ${result.type === 'Product' ? 'bg-[var(--color-brand)]/20 text-[var(--color-brand)]' : 'bg-gray-200 text-gray-600'}`}>
