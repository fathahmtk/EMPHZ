    ...
    <section className="py-24 bg-[var(--color-surface)]">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          {/* Blog Articles */}
          <div>
            <h3 className="text-2xl font-semibold mb-6 text-[var(--color-primary)]">Latest Articles</h3>
            <div className="space-y-6">
              {BLOG_ARTICLES.map((article, index) => (
                <a key={index} href={article.link} className="block p-6 bg-[var(--color-background)] rounded-lg shadow-[var(--shadow-md)] hover:shadow-[var(--shadow-xl)] border border-[var(--color-border)] transition-all duration-300 group hover:-translate-y-1">
                  ...
                  <p className="text-[var(--color-secondary)] text-sm">{article.description}</p>
          ...
          <div className="bg-[var(--color-background)] p-8 rounded-lg shadow-[var(--shadow-md)] border border-[var(--color-border)]">
            <h3 className="text-2xl font-semibold mb-6 text-[var(--color-primary)]">Downloadable Resources</h3>
            <div className="space-y-4">
              {KNOWLEDGE_RESOURCES.map((resource, index) => (
                <Button key={index} variant="secondary" href={resource.link} className="w-full justify-center">
                  {resource.name}
                </Button>
              ))}
            </div>
